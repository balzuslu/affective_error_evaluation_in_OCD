...
```{r fast-hit-response-time-limit}

# Define data basis (all go trials, equivalent to calculation in task program) 
single_trial_data_rt_limit <- single_trial_data %>%
  dplyr::filter(
    gng_response_type != "FA" & 
    gng_response_type != "IR" &
    gng_response_type != "miss" &
    gng_response_type != "wrong_key" & 
    stimulation == "sham")
...
# Add block variable
single_trial_data_rt_limit <- single_trial_data_rt_limit %>%
...
mean_rt_per_block <- single_trial_data_rt_limit %>%
  dplyr::group_by(participant_id, block) %>%
  dplyr::summarize(mean_rt = mean(gng_rt)) %>%
  dplyr::ungroup()


# Calculate fast hit response time limit in calibration blocks 1, 4, and 7
...
# Calculate mean, sd, min, and max of fast hit response time limit per group
...
# Display table 
my_table_template(fh_time, caption = "Fast Hit Response Time Limit",
